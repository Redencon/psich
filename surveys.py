...
class Survey:
    ...
    def parse_response(self, uid: int, data: str) -> int:
        m = re.match(r"SA_([A-Za-z]+)_([0-9]+)_([0-9]+)", data)
        if m is None:
            return -1
    ...
    def is_complete(self) -> bool:
        for uid in self.participants:
            if uid not in self.answers:
                return False
            if self.answers[uid] is None:
                continue
            if any([ans is None for ans in self.answers[uid]]):
                return False
        return True
    ...
    def dump(self):
        with open(self.filename, "w") as file:
            json.dump(self.answers, file, indent=2)
...
class SurveyPool:
    ...
    def spawn_survey(self, *, code: str = None, config_filename: str = None, **kwargs):
        assert code is not None or config_filename is not None
        filename = config_filename if code is None else "{}.json".format(code)
    ...
    def user_declined(self, code, uid):
        self.active_surveys[code].answers[uid] = None
        self.active_surveys[code].dump()
        self.dump_surveys()
