...
class Pending_users():
    def __init__(self):
        with open('pending.json') as file:
...
def parse_survey(call: types.CallbackQuery):
    ...
    for i in (3, 7, 14, 30, 61, 150):
        a = streak_achievement(call.from_user.id, i)
        if a is not None:
            if a:
...
def unsub_check(call):
    if call.data[-1] == 'y':
        ...
        dab_upd('status.json', call.from_user.id, TIMES[1])
...
def stats(message: types.Message):
    ...
    if add_achievement(message.from_user.id, 'stats'):
...
def today(message):
    if add_achievement(message.from_user.id, 'today'):
    ...
    if response.isdigit():
        if user_db['code'] is not None:
            if int(response) == user_db['code']:
                ...
                dab_upd('status.json', message.from_user.id, None)
                wanna_get(message)
                return
            else:
...
def grant(message: types.Message):
    ...
    if text[1] not in gen_data.keys():
        return
    user = int(text[0])
    if add_achievement(user, text[1]):
...
def new_operator(message: types.Message):
    for user in message.new_chat_members:
        if add_achievement(user.id, 'operator'):
